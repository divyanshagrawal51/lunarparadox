...
def main():
    ...
    test_pred_sum = np.zeros(test_mat.shape[0])
    for itr, iva in skf.split(oof_mat, y):
        lr = LogisticRegression(C=1.0, max_iter=1000)
        lr.fit(oof_mat[itr], y[itr])
        stack_oof[iva] = lr.predict_proba(oof_mat[iva])[:, 1]
        test_pred_sum += lr.predict_proba(test_mat)[:, 1]
    test_pred = test_pred_sum / skf.n_splits

    thr, sc = best_threshold(y, stack_oof)
    print(f"\nstacked OOF bal-acc = {sc:.4f} (thr={thr:.3f})")

    simple_avg_oof = oof_mat.mean(axis=1)
    _, sc_avg = best_threshold(y, simple_avg_oof)
    print(f"simple average OOF bal-acc = {sc_avg:.4f}")

    if sc > sc_avg:
        print("\nstacking wins, using it")
        final_test, final_thr = test_pred, thr
    else:
        print("\nsimple average was as good or better, using that")
        final_test, final_thr = test_mat.mean(axis=1), best_threshold(y, simple_avg_oof)[0]
    ...
    sub = pd.DataFrame({"image_id": te[id_c].astype(str).values,
                        "label": (final_test > final_thr).astype(int)})
